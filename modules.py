...
def KNN(point_xyz, center_xyz, K):
    distances = torch.cdist(point_xyz, center_xyz)  # (batchsize, N1, N2)
    # _, indices = torch.topk(distances, K, dim=-1, largest=False, sorted=False)  # (batchsize, N1, K)
    _, indices = torch.topk(distances, K, dim=1, largest=False, sorted=False)
...
class Group(nn.Module):  # FPS + KNN
    ...
    def forward(self, xyz):
        ...
        idx = KNN(xyz, center, self.group_size)  # B G M
