...
def process_file(file_paths):
    ...
    root= 'CBCT_Mesh_data/train_data_npy/' 
    ...
    # 下颌骨mesh加载  非必须，只在训练ConfidenceNet 加权曲线拟合的时候有用
    ...
    sampled_points_t, normals_t = sample_points_from_meshes(bone_meshes, target_num, return_normals=True)
...
def main():
    root = './CBCT_Mesh_data/'
    bone_paths = list(pathlib.Path(root + "train_data/").glob('*_bone1.vtk'))
    ply_paths = [pathlib.Path(root + "train_data/" + i.stem[:-6] + '.ply')  for i in bone_paths]
    ctr_curve_list = [pathlib.Path(root + "train_data/" + i.stem[:-6] + '-ctr.txt')  for i in bone_paths]
    allp_curve_list = [pathlib.Path(root + "train_data/" + i.stem[:-6] + '-all.txt')  for i in bone_paths]
